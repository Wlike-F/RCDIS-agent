...
-- ---------- Reimbursement guide (structured policy knowledge, NOT RAG) ----------
-- Deterministic, versioned SOP per expense category: ordered steps + required materials.
-- The Agent reads this through a typed read-only tool so policy answers are exact and auditable.
create table if not exists reimbursement_guide
(
    id                bigserial primary key,
    ...
    created_at        timestamptz  not null default now(),
    updated_at        timestamptz  not null default now(),
    constraint uk_reimbursement_guide_category unique (category)
);

-- Idempotent seeds: re-running never duplicates and never overrows an edited guide.
INSERT INTO reimbursement_guide (category, title, steps_json, materials_json, notes, effective_date)
SELECT '通用', '通用报销流程',
       '["在报销中心新建报销单并逐条填写支出明细（含发票/凭证）","核对所需材料齐全","提交审批","审批通过后由财务打款"]',
       '["发票","费用明细或清单","审批单"]',
       '材料不全时系统会落草稿并提示缺项，补齐后再提交。',
       '2026-01-01'
WHERE NOT EXISTS (SELECT 1 FROM reimbursement_guide WHERE category = '通用');

INSERT INTO reimbursement_guide (category, title, steps_json, materials_json, notes, effective_date)
SELECT '差旅费', '差旅费报销流程',
       '["出行前提交出差审批","行程结束后登记差旅支出并上传票据","创建报销单关联差旅支出","核对差旅材料齐全","提交审批"]',
       '["交通票据或行程单","住宿发票","出差审批单","发票"]',
       '住宿与交通标准按实验室差旅规定执行，超标部分需说明理由。',
       '2026-01-01'
WHERE NOT EXISTS (SELECT 1 FROM reimbursement_guide WHERE category = '差旅费');

INSERT INTO reimbursement_guide (category, title, steps_json, materials_json, notes, effective_date)
SELECT '耗材费', '耗材费报销流程',
       '["采购前确认预算科目余额","登记耗材支出并上传发票","办理入库或验收","创建报销单关联支出","提交审批"]',
       '["发票","采购合同或订单","入库单或验收单"]',
       '大额耗材需附合同；验收单须有经办人签字。',
       '2026-01-01'
WHERE NOT EXISTS (SELECT 1 FROM reimbursement_guide WHERE category = '耗材费');

INSERT INTO reimbursement_guide (category, title, steps_json, materials_json, notes, effective_date)
SELECT '设备费', '设备费报销流程',
       '["采购前立项并确认预算","登记设备支出并上传发票","设备到货验收","办理固定资产登记","创建报销单关联支出并提交审批"]',
       '["发票","采购合同","验收单","固定资产登记表"]',
       '达到固定资产标准的设备必须先登记资产再报销。',
       '2026-01-01'
WHERE NOT EXISTS (SELECT 1 FROM reimbursement_guide WHERE category = '设备费');

INSERT INTO reimbursement_guide (category, title, steps_json, materials_json, notes, effective_date)
SELECT '测试化验加工费', '测试化验加工费报销流程',
       '["签订委托合同或协议","登记测试支出并上传发票","取得测试报告或成果证明","创建报销单关联支出","提交审批"]',
       '["发票","委托合同或协议","测试报告或成果证明"]',
       '外委测试需附合同与成果证明，否则不予报销。',
       '2026-01-01'
WHERE NOT EXISTS (SELECT 1 FROM reimbursement_guide WHERE category = '测试化验加工费');
